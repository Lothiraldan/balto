...
class TestSuite():
    ...
    async def new_output(self, data):
        if data.get("_type") in ("test_collection", "test_result") and data['suite_name'] == self.name:
            test_id = data['id']
    ...
    async def collect_all(self, directory, event_emitter, loop):
        session = self.get_runner(directory, event_emitter, [], loop=loop,
                                  collect_only=True, suite_name=self.name)
        await session.run()

    async def launch_all(self, directory, event_emitter, loop):
        session = self.get_runner(directory, event_emitter, [], loop=loop, suite_name=self.name)
        await session.run()

    async def launch_tests(self, directory, event_emitter, loop, tests):
        session = self.get_runner(directory, event_emitter, tests, loop=loop, suite_name=self.name)
        await session.run()